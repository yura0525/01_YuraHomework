	...
	T_STR szShaderName, T_STR szTexName, T_STR VSFunc, T_STR PSFunc)
{
	...
	xObject::Create(pd3dDevice, texMaxU, texMaxV, xPos, yPos, left, top, I_GameDataLoad.g_HPBAR_WIDTH, I_GameDataLoad.g_HPBAR_HEIGHT, szShaderName,
...
bool  THPBarObject::Frame()
{
	...
	m_rtDraw.right = I_GameDataLoad.g_HPBAR_WIDTH * percent;
	
	float newXPos = m_pOwner->m_pos.x - ((I_GameDataLoad.g_HPBAR_WIDTH - m_rtDraw.right) /2);
	float newYPos = m_pOwner->m_pos.y + I_GameDataLoad.g_INIT_HERO_HP_GAP_POSY;
